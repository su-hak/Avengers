...
// Q 스킬 데미지 레벨별 업데이트
// 스킬 레벨별 버튼 선언
const SkillLevelButtons = document.querySelectorAll("#skill1-level1, #skill1-level2, #skill1-level3, #skill1-level4, #skill1-level5");
// 현재 레벨 1
let currentLevel = 1; // Set initial level

// 스클레벨별 버튼 클릭시 데미지 불러오기
    SkillLevelButtons.forEach(button => {
        button.addEventListener("click", function () {
            currentLevel = parseInt(button.innerText); // Get the level from the button text
            updateLuxQDamage();
        });
    });
...
// 스킬Q 레벨별 데이지 업데이트
    function updateLuxQDamage() {
        const LuxQDamage = (index === 74) ? championsArray[index].abilities.Q[0].effects[0].leveling[0].modifiers[0].values[currentLevel - 1] : '';
